...
def get_editor(block):
    if block:
        user_choice = gc("editor blocking")
        params = gc("editor blocking parameters")
    else:
        user_choice = gc("editor non-blocking")
        params = gc("editor non-blocking parameters")
    if not gc("editor test for avalability and auto chose available editor"):
